...
const getVideoById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!video) {
      throw new ApiError(
        500,
        "video with same id was not found or something went wrong while fetching"
      );
    }
    return res
      .status(200)
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  try {
    if (!isValidObjectId(videoId)) {
      throw new ApiError(404, "invalid video id please.");
    }
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!ownerId.owner.toString() === req.user._id.toString()) {
      throw new ApiError(
        410,
        "you are unAuthorized , only the owner can delete"
      );
    }
    ...
    if (
      (deleteVideoFromCloudinary && deleteThumbnailFromCloudinary) ==
      "not found"
    ) {
      throw new ApiError(
        404,
